...
raw_counts <- read.csv("merged_counts_matrix.csv", header = TRUE, row.names = "Geneid", stringsAsFactors = FALSE)
head(raw_counts)
...
counts_gse <- read.csv("merged_counts_matrix.csv",
                       header = TRUE,
...
output_file <- "biotype_count_matrix.csv"
...
png("D:\\RNAseq Analysis\\de_gene_heatmap.png",
...
png("D:\\RNAseq Analysis\\density_plots_raw_vst.png",
...
hallmark_pathway <- split(x = m_df$gene_symbol, f = m_df$gs_name)
head(names(hallmark_pathway))
#hallmark_pathway <- gmtPathways("D:\\RNAseq Analysis\\h.all.v7.0.symbols.gmt.txt")

library(tibble)
library(dplyr)
library(clusterProfiler)
library(org.Hs.eg.db)
# 1. Convert Ensembl IDs to Symbols
# This creates a mapping table
id_map <- bitr(lncap_ranked_df$Gene.name, 
               fromType = "ENSEMBL", 
               toType   = "SYMBOL", 
               OrgDb    = org.Hs.eg.db)

# 2. Join the symbols back to your fold change data
lncap_ranked_symbols <- lncap_ranked_df %>%
  inner_join(id_map, by = c("Gene.name" = "ENSEMBL")) %>%
  dplyr::select(SYMBOL, log2FoldChange) %>%
  rename(Gene.name = SYMBOL)

# 3. Now run your preparation function on the SYMBOL version
lncap_ranked_list <- prepare_ranked_list(lncap_ranked_symbols)

# 4. Verify - you should see symbols now!
...
# Find the specific name for Hypoxia in your list
target_pathway <- "HALLMARK_HYPOXIA"

png("fgsea_hypoxia.png", width = 1200, height = 900, res = 200)

plotEnrichment(hallmark_pathway[[target_pathway]], lncap_ranked_list) + 
  labs(title = target_pathway, 
       subtitle = "LNCaP: Hypoxia vs Normoxia")
